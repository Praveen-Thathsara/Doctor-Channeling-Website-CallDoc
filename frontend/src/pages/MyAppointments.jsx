  ...
  return (
    <div>
      ...
      {appointments.length === 0 ? (
        <p className="text-center text-zinc-500 py-10">No appointments found.</p>
      ) : (
        appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            <div>
              <img className='w-32 bg-[#00A4A620]' src={item.docData?.image} alt="" />
            </div>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && !item.payment && !item.isCompleted && (
                <button className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border hover:bg-[#00A4A6] hover:text-white transition-all duration-300'>
                  Pay Online
                </button>
              )}
